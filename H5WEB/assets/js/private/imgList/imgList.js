require(['jquery','loaddh','comm'],function(){
 ...
 var wrap = $('#content');
   ...
   var timeess=new Date().getTime();
    ...
    var url=localPost+'coll?collId='+uid+'&typeInside=1&currentPage='+pageIndex+'&screenDPI='+wrap_width+'&timeess='+timeess;
    loads(url);
        //翻页
        $(window).on('scroll',function(){
          if($(document).height()-$(document).scrollTop()<=$(window).height()){
           setTimeout(function(){
            loads(localPost+'coll?collId='+uid+'&typeInside=1&currentPage='+pageIndex+'&screenDPI='+wrap_width+'&timeess='+timeess);
          },300)
         }
       })
        function loads(url){
